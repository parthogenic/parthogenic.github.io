...
define([
  ...
  function (
    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      _initSelectionGL: function() {
        this.selectionGL = new GraphicsLayer();
        this.selectionGL.name = "Search Results Selection";
        this.map.addLayer(this.selectionGL);
      },
      ...
      _setPopupMenuItems: function() {
        ...
        if(!featureSet.geometryType && layer.graphics[0]){
          var geomType = "";
        ...
        this.featureActionManager.getSupportedActions(featureSet, layer).then(lang.hitch(this, function(actions){
          ...
          if(this.list.hasSelectedItem()){
            var removeAction2 = new BaseFeatureAction({
              name: "ClearSelctedeSearchResult",
              iconClass: 'icon-close',
              label: this.nls.clearSelectedResults,
              iconFormat: 'svg',
              map: this.map,
              onExecute: lang.hitch(this, this.clearSelection)
            });
            removeAction2.name = "ClearSelectedeSearchResult";
            removeAction2.data = featureSet;
            actions.push(removeAction2);
          }

          if(this.relateLayers && this.relateLayers.length > 0){
            var removeAction3 = new BaseFeatureAction({
            ...
            removeAction3.name = "ClearRelateResult";
            removeAction3.data = featureSet;
            actions.push(removeAction3);
          }
      ...
      _addBufferLayer: function () {
        ...
        if (this.config.bufferDefaults.addtolegend) {
          ...
          var layerInfo = {
            ...
            "name": this.nls.search + ' ' + this.nls.buffergraphics + ' ' + this.nls.results,
          ...
          this.graphicsLayerBuffer.name = this.nls.search + ' ' + this.nls.buffergraphics + ' ' + this.nls.results;
          this.graphicsLayerBuffer.setRenderer(renderer);
        } else {
          //use graphics layer
      ...
      _removeSelectionGraphics: function () {
...
//            console.info("spatial layer index: " + this.spatialLayerIndex);
      ...
      _initTabContainer: function () {
        ...
        this.mouseoverlist = this.config.mouseoverlist || false;
        ...
        if(this.autoactivatedtool && initView === "graphical" && this.shapeTab){
        ...
        if (this.mouseoverlist) {
          this.own(on(this.list, "mouseover", lang.hitch(this, this._overResultItem)));
          this.own(on(this.list, "mouseout", lang.hitch(this, this._outResultItem)));
        }
      },
      ...
      _initLayerSelect: function () {
        ...
        if (graOptions.length > 0) {
          graOptions[0].selected = true;
        }
          ...
          all(defs).then(lang.hitch(this, function (results) {
            ...
            array.forEach(results, lang.hitch(this, function (result, j) {
              if(result.state === 'success'){
                ...
                var aIndex = -1, gIndex = -1, sIndex = -1;
                array.some(attribOptions, lang.hitch(this, function(option, a){
                ...
                array.some(graOptions, lang.hitch(this, function(option, g){
                  if(option.label === this.config.layers[j].name){
                    gIndex = g;
                    return true;
                  }
                ...
                array.some(spatialOptions, lang.hitch(this, function(option, s){
                  if(option.label === this.config.layers[j].name){
                    sIndex = s;
                    return true;
                  }
                ...
                if(aIndex > -1){
                  attribOptions.splice(aIndex, 1);
                }
                if(gIndex > -1){
                  graOptions.splice(gIndex, 1);
                }
                if(sIndex > -1){
                  spatialOptions.splice(sIndex, 1);
                }
      ...
      _removeResultItem: function (index, item) {
        if(item === this.list.getSelectedItem()){
      ...
      buildWhereClause: function (layerIndex, expressIndex, /* optional */ theValue) {
        ...
        for (var s = 0; s < content.length; s++) {
          ...
          if (content[s].value.toString().toLowerCase().trim() === "all") {
            ...
            //|| content[s].value.toString().toLowerCase() === "nan"
//Change in 2.9 to see if we can allow null values back in the the unique dropdown
      ...
      _onSearchFinish: function (layerIndex, closeOnComplete, removing, adding, results) {
        ...
        if(this.resultLayers[this.currentLayerIndex].type.toLowerCase() !== "table"){
          //Check if layer is a table or it has geometry
...
//modify the currentFeatures with the new results
        ...
        for (i = 0; i < len; i++) {
          ...
          var content = "",
            ...
            oidVal;
          csvColumns = [];
          //ensure fields are ordered the same way they are configured in the json (this is an issue for ArcGIS Server 10.2.x)
            ...
            if (titlefield && upperCaseFieldName === titlefield.toUpperCase()) {
              ...
              if (this._isVisible(att, layerIndex)) {
                if(lyrHideNullValues && value === ""){
                  ...
                  content = content + this.resultFormatString.replace('[attribname]', this._getAlias(att, layerIndex)).replace('[attribvalue]', value).replace('::', ': ');
          ...
          if(!currentLayer.renderer){
            if (this.config.symbols && this.config.symbols.picturemarkersymbol) {
              ...
              pms.url = (pms.url.indexOf(this.folderUrl) < 0) ? this.folderUrl + pms.url : pms.url;
        ...
        if (symFromWhere === 'server') {
          ...
          if(symFromWhere === 'layer'){
            ...
            if(layerConfig.symbology){
              var simpleJson0 = {
                'type': 'simple',
                'label': this.config.layers[this.currentLayerIndex].name,
                'symbol': layerConfig.symbology
              };
              var sRend = new SimpleRenderer(simpleJson0);
              sRend.description = this.config.layers[this.currentLayerIndex].name;
              return sRend;
            }
          }
          ...
          var simpRend = new SimpleRenderer(simpleJson);
          simpRend.description = this.config.layers[this.currentLayerIndex].name;
          return simpRend;
        }
      },
      ...
      _addSelectionGra: function(item, hover) {
        //add the selection symbol
        var selClr = this.map.infoWindow.lineSymbol.color;
        if(this.list.items.length > 1){
          var gSym = this.currentSearchLayer.renderer.getSymbol(item.graphic);
          var selSym;
          this.map.reorderLayer(this.selectionGL, 500);
          var selGra = new Graphic(item.graphic.toJson());
          if(hover){
            selGra.attributes.selType = 'hover';
          }else{
            selGra.attributes.selType = 'selection';
          }
          if (item.graphic.geometry.type === "point"){
            var size = this._getPointSymbolSize(gSym);
            selSym = new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_SQUARE, size,
              new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, selClr, 2),
              new Color([selClr.r,selClr.g,selClr.b, (hover)?0.2:0]));
          }else{
            if(gSym.hasOwnProperty('outline')){
              //this is a SimpleFillSymbol
              selSym = new SimpleFillSymbol();
              if(!hover){
                selSym.setStyle(SimpleFillSymbol.STYLE_NULL);
              }else{
                selSym.setColor(new Color([selClr.r,selClr.g,selClr.b, 0.2]));
              }
              selSym.outline.setColor(selClr);
              selSym.outline.setWidth(3);
            }else{
              //this is a SimpleLineSymbol
              selSym = new SimpleLineSymbol();
              selSym.setColor(selClr);
              selSym.setWidth(3);
            }
          }
          if(hover){
            this._clearHoverSelectionGras();
          }else{
            this.selectionGL.clear();
          }
          var simpleJson = {
            'type': 'simple',
            'label': this.nls.search + ' ' + this.nls.selectiongraphics,
            'symbol': selSym
          };
          var renderer = new SimpleRenderer(simpleJson);
          this.selectionGL.setRenderer(renderer);
      ...
      _searchResultListByOID: function (OID) {
        ...
        for (var i = 0; i < this.list.items.length; i++) {
          ...
          if (item.OID === OID) {
            ...
            if(itemDom){
              itemDom.scrollIntoView(false, false);
            }
            this._addSelectionGra(item, false);
            this.list.setSelectedItem(this.list.id.toLowerCase() + item.id);
            if ((this.map.infoWindow && (this.config.enablePopupsOnResultClick || this.config.mouseovergraphics)) && !lyrHasPopupDisabled) {
      ...
      _clearHoverSelectionGras: function() {
        for (var g = this.selectionGL.graphics.length - 1; g >= 0; g--){
          var gra = this.selectionGL.graphics[g];
          if(gra.attributes.selType === 'hover'){
            this.selectionGL.remove(gra);
          }
        }
      },
      ...
      _addCustomStyle: function(theme) {
        ...
        cssText += ".FoldableTheme.yellow .widget-esearch .search-btn,";
        cssText += ".TabTheme.yellow .widget-esearch .search-btn,";
        cssText += ".JewelryBoxTheme.yellow .widget-esearch .search-btn,";
        cssText += ".LaunchpadTheme.style3 .widget-esearch .search-btn,";
        ...
        cssText += "}";
        cssText += ".FoldableTheme.yellow .widget-esearch .search-btn:hover,";
        cssText += ".TabTheme.yellow .widget-esearch .search-btn:hover,";
        cssText += ".JewelryBoxTheme.yellow .widget-esearch .search-btn:hover,";
        cssText += ".LaunchpadTheme.style3 .widget-esearch .search-btn:hover,";
